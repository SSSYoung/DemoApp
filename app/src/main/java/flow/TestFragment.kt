...
class TestFragment : Fragment(R.layout.fragment_test) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        observeEvent<GlobalEvent>(isSticky = true) {
            Log.d(TAG, "TestFragment received GlobalEvent 2:${it.name}")
        }

        observeEvent<GlobalEvent>(dispatcher = Dispatchers.IO) {
            Log.d(TAG, "TestFragment received GlobalEvent 3:${it.name}")
        }


        //监听Activity事件
        observeEvent<ActivityEvent>(scope = requireActivity(), isSticky = true) {
            Log.d(TAG, "TestFragment received FragmentEvent 2:${it.name}")
        }
        ...
        observeEvent<ActivityEvent>(
            ...
            Log.d(TAG, "TestFragment received FragmentEvent 3:${it.name}")
        }

        observeEvent<ActivityEvent>(scope = requireActivity(), Dispatchers.IO) {
            Log.d(TAG, "TestFragment received FragmentEvent 4:${it.name}")
        }
        ...
        observeEvent<ActivityEvent>(
            ...
            Log.d(TAG, "TestFragment received FragmentEvent 5:${it.name}")
        }

        //监听自己Scope事件
        observeEvent<FragmentEvent>(scope = this, Dispatchers.IO, Lifecycle.State.STARTED) {
            Log.d(TAG, "TestFragment received FragmentEvent 1:${it.name}")
        }
